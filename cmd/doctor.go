...
var doctorCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if err != nil {
			...
			fmt.Println("  ✅ Notification System: Ready (Native Desktop Engine)")
		}

		// 5. Assets
		icon, sound, err := assets.EnsureAssetsWritten()
		if err != nil {
			fmt.Printf("  ⚠️  Assets:          Error: %v\n", err)
		} else {
			fmt.Printf("  ✅ Assets:          Icon: %s\n", icon)
			fmt.Printf("                      Sound: %s\n", sound)
		}
